...
export const Scene: React.FC<SceneProps> = ({
  ...
  useEffect(() => {
    if (resetTrigger > 0) {
      ...
      if (controlsRef.current && cameraRef.current) {
        ...
        cameraRef.current.position.set(17, 12.47, 16.78);
  ...
  useEffect(() => {
    ...
    if (!isIntroActiveRef.current) {
      camera.position.set(17, 12.47, 16.78);
    } else {
    ...
    registerDraggable(phoneGroup, [-7.15, -1.24, 4.09], baseTilt, [0, 0, -1.0], 1.0, 0.04, 0.015, "phone", "Smartphone 3D Mockup");
    ...
    registerDraggable(piwvaTextGroup, [0.15, 1.3, 1.98], [-0.55, 0.38, 0.35], [1.6, 2.2, 1.5], 1.7, 0.07, 0.025, "piwva_logo", "Piwva 3D Logo Letters");
    ...
    registerDraggable(designedLogoGroup, [3.17, 2.96, -2.61], baseTilt, [0, 1.0, 1.0], 1.4, 0.08, 0.03, "designed_logo_circle", "Piwva Designed Logo Medallion");
    ...
    registerDraggable(videoGroup, [-4.8, 3.76, 2.2], baseTilt, [-1.2, 1.0, 1.0], 1.3, 0.07, 0.025, "video_screen_circle", "Piwva Baby Video Reel");
    ...
    registerDraggable(palCard, [5.25, 0.63, 1.28], [-0.56, 0.38, 0.36], [-2.6, 1.8, 1.4], 1.3, 0.08, 0.03, "color_palette", "Nursery Climate Card");
    ...
    registerDraggable(motorCard, [5.82, 2.92, -5.65], [-0.58, 0.4, 0.38], [-2.6, -1.6, 1.8], 1.5, 0.1, 0.035, "product_card", "Motor & Sensory Health Card");
    ...
    snGroup.add(snFace);
    registerDraggable(snGroup, [2.42, 5.07, 1.44], [-0.56, 0.38, 0.36], [2.4, 1.2, 1.6], 1.4, 0.06, 0.02, "shop_now_btn", "Health Guide Pill Button");
    ...
    registerDraggable(actsGroup, [-6.27, -3.2, 5.98], baseTilt, [-1.5, -1.2, 1.0], 1.1, 0.05, 0.02, "action_buttons", "Health Action Icons");
    ...
    registerDraggable(blocksGroup, [-4.7, -4.57, 2], [-0.45, 0.35, 0.25], [-3.0, 0.5, 1.8], 1.6, 0.09, 0.03, "wooden_blocks", "Montessori Wooden Blocks");
    ...
    registerDraggable(rattleGroup, [4.8, -4.73, 1.8], [-0.48, 0.36, 0.3], [3.0, 2.2, 1.6], 1.8, 0.08, 0.025, "baby_rattle", "Baby Rattle & Teether");
    ...
    registerDraggable(laughingCard, [5.5, -7.57, 3.2], baseTilt, [-1.2, 1.0, 1.0], 1.4, 0.08, 0.03, "card_laughing_photo", "Laughing Baby Photo Card");

    // 12. Ring Stacker
    const ringStacker = create3DRingStacker();
    registerDraggable(ringStacker, [-0.2, -8.13, 4.2], [-0.4, 0.35, 0.2], [0, 1.2, 1.0], 1.5, 0.08, 0.03, "ring_stacker_3d", "3D Wooden Ring Stacker");
    ...
    registerDraggable(sleepTipsCard, [4.8, -14.94, 1.5], baseTilt, [-1.5, -1.0, 1.2], 1.4, 0.07, 0.025, "card_sleep_tips", "Safe Sleep Guidelines Card");

    // 15. Moon Mobile
    const moonMobile = create3DMoonMobile();
    registerDraggable(moonMobile, [-0.2, -16.13, 4], [-0.4, 0.3, 0.2], [0, 1.5, 1.0], 1.6, 0.09, 0.03, "moon_nightlight", "3D Moon & Stars Mobile");

    // 16. Climate Medallion (Bengali)
    const tempBadge = create3DCylinderMedallion("🌡️", "২০–২২°C", "৪৫–৫৫% আর্দ্রতা", "শ্বাসনালী সুরক্ষা");
    registerDraggable(tempBadge, [8.2, -13.94, -1.5], baseTilt, [-1.0, 1.2, 1.0], 1.2, 0.08, 0.02, "temp_badge", "Climate & Humidity Medallion");
    ...
    registerDraggable(swaddleCard, [-7.8, -19.57, 1], baseTilt, [1.0, 1.0, 1.0], 1.3, 0.07, 0.02, "swaddle_guide_card", "Swaddle Safety Guide Card");
    ...
    registerDraggable(lampGroup, [1.5, -20.27, 3.5], [-0.3, 0.2, 0.1], [0, 1.0, 1.0], 1.5, 0.07, 0.02, "circadian_lamp_3d", "3D Nursery Nightlight Lamp");
    ...
    registerDraggable(tummyPhotoCard, [5.2, -27.03, 2.8], baseTilt, [-1.5, 1.0, 1.0], 1.4, 0.08, 0.03, "card_tummy_photo", "Tummy Time Baby Photo Card");
    ...
    registerDraggable(tummyTipsCard, [1.26, -29.33, 2.43], baseTilt, [1.5, -1.0, 1.2], 1.3, 0.07, 0.025, "card_tummy_tips", "Tummy Time Milestone Card");
    ...
    registerDraggable(crawlingPhotoCard, [-6.8, -32.42, 1.5], baseTilt, [1.2, 1.0, 1.0], 1.4, 0.08, 0.03, "card_crawling_photo", "Crawling Baby Photo Card");
    ...
    registerDraggable(reviewCard, [6.5, -33.47, 0.5], baseTilt, [-1.2, 1.0, 1.0], 1.2, 0.07, 0.02, "parent_review", "Pediatric Endorsement Card");
    ...
    registerDraggable(contrastCard, [0, -34.74, 3.8], baseTilt, [0, 1.0, 1.0], 1.5, 0.08, 0.025, "contrast_card_3d", "High-Contrast Visual Card");
    ...
    registerDraggable(abacus3D, [-4.5, -36.45, 2.5], baseTilt, [1.0, 0.5, 1.0], 1.6, 0.08, 0.03, "wooden_abacus_3d", "3D Montessori Abacus Model");
    ...
    registerDraggable(m3Badge, [4.8, -37.26, 1.2], baseTilt, [-1.0, 1.0, 1.0], 1.3, 0.07, 0.02, "milestone_3m_card", "3-Month Milestones Badge");

    const m6Badge = create3DCylinderMedallion("🌟", "৬ মাসের লক্ষ্য", "সাপোর্ট নিয়ে বসা", "মোটর মাইলস্টোন", true, 0.75);
    registerDraggable(m6Badge, [-0.5, -38.43, 3.5], baseTilt, [0, 1.0, 1.0], 1.4, 0.08, 0.025, "milestone_6m_card", "6-Month Milestones Badge");
    ...
    registerDraggable(teethingPhotoCard, [-5.4, -44.07, 3.2], baseTilt, [1.5, 1.0, 1.0], 1.4, 0.08, 0.03, "card_teething_photo", "Teething Baby Photo Card");
    ...
    registerDraggable(teethingTipsCard, [4.9, -45.13, 2], baseTilt, [-1.5, -1.0, 1.2], 1.3, 0.07, 0.025, "card_teething_tips", "Teething Guidelines Card");

    // 29. Silicone Chain
    const siliconeChain = create3DSiliconeChain();
    registerDraggable(siliconeChain, [0.1, -46.45, 4], [-0.4, 0.35, 0.2], [0, 1.0, 1.0], 1.5, 0.08, 0.025, "silicone_beads_3d", "3D Silicone Teether Chain");

    // 30. Eco Badge (Bengali)
    const ecoBadge = create3DCylinderMedallion("🌱", "১০০% নিরাপদ", "এফএসসি বিচউড", "টক্সিক উপাদান-মুক্ত");
    registerDraggable(ecoBadge, [8.4, -44.44, -1], baseTilt, [-1.0, 1.2, 1.0], 1.5, 0.08, 0.025, "eco_badge", "100% Newborn Safe Health Seal");
    ...
    registerDraggable(solidsCard, [-7.2, -50.57, 1.2], baseTilt, [1.0, 1.0, 1.0], 1.3, 0.07, 0.02, "solids_guide_card", "Weaning & Solids Guide Card");
    ...
    registerDraggable(oralWipeCard, [0, -52.88, 3.6], baseTilt, [0, 1.0, 1.0], 1.4, 0.08, 0.025, "oral_wipe_card", "Oral & Gum Care Card");
    ...
    registerDraggable(starToyGroup, [-4.5, -54.46, 2.5], baseTilt, [-0.5, 1.0, 1.0], 1.5, 0.08, 0.025, "silicone_star_toy", "3D Silicone Star Teether");
    ...
    registerDraggable(motherPhotoCard, [5, -60.93, 3], baseTilt, [-1.5, 1.0, 1.0], 1.4, 0.08, 0.03, "card_mother_photo", "Kangaroo Care Baby Photo Card");
    ...
    registerDraggable(colicTipsCard, [-5.2, -62.55, 2.5], baseTilt, [1.5, -1.0, 1.2], 1.3, 0.07, 0.025, "card_colic_tips", "Colic & Gas Relief Card");

    // 37. Cloud Mobile
    const cloudMobile = create3DCloudMobile();
    registerDraggable(cloudMobile, [0, -64.06, 4], [-0.4, 0.35, 0.2], [0, 1.0, 1.0], 1.5, 0.08, 0.03, "cloud_mobile_3d", "3D Cloud Nursery Mobile");
    ...
    registerDraggable(newsCard, [8.2, -61.53, -0.5], baseTilt, [-1.0, 1.2, 1.0], 1.2, 0.09, 0.03, "newsletter_card", "Pediatrician Health Tips Card");
    ...
    registerDraggable(burpingCard, [-7.5, -67.46, 1.5], baseTilt, [1.0, 1.0, 1.0], 1.3, 0.07, 0.02, "burping_guide_card", "Burping Techniques Guide Card");
    ...
    registerDraggable(massageCard, [6.5, -68.21, 1.2], baseTilt, [-1.0, 1.0, 1.0], 1.4, 0.08, 0.025, "massage_oil_card", "Infant Massage Routine Card");
    ...
    registerDraggable(heartShieldGroup, [-4.5, -70.98, 2.8], baseTilt, [-0.5, 1.0, 1.0], 1.6, 0.08, 0.03, "heart_shield_3d", "3D Heart Health Shield");
    ...
    registerDraggable(feverCard, [-5.5, -77.03, 3], baseTilt, [1.2, 1.0, 1.0], 1.3, 0.08, 0.03, "fever_protocol_card", "Infant Fever Protocol Card");
    ...
    registerDraggable(dehydrationCard, [5.2, -78.27, 2], baseTilt, [-1.2, 1.0, 1.0], 1.4, 0.07, 0.025, "dehydration_card", "Dehydration Warning Signs Card");
    ...
    registerDraggable(breathingCard, [0, -81.43, 3.8], baseTilt, [0, 1.0, 1.0], 1.5, 0.08, 0.025, "breathing_card", "Respiratory Distress Card");
    ...
    registerDraggable(stethBadge, [6.8, -84.25, 1.2], baseTilt, [-1.0, 1.0, 1.0], 1.3, 0.07, 0.02, "stethoscope_badge", "Clinical Advisory Badge");
    ...
    registerDraggable(kitGroup, [-0.2, -86.56, 3.6], [-0.4, 0.3, 0.2], [0, 1.0, 1.0], 1.5, 0.08, 0.025, "first_aid_kit_3d", "3D Nursery First Aid Box");
    ...
    registerDraggable(fscSeal, [-5.5, -91.92, 3], baseTilt, [1.0, 1.0, 1.0], 1.4, 0.08, 0.025, "fsc_wood_seal", "FSC Beechwood Certified Seal");

    // 50. BPA-Free Seal (Bengali)
    const bpaSeal = create3DCylinderMedallion("🛡️", "বিপিএ-মুক্ত", "ল্যাব টেস্টে নিরাপদ", "শিশুর শতভাগ সুরক্ষা");
    registerDraggable(bpaSeal, [5.5, -93.21, 2], baseTilt, [-1.0, 1.0, 1.0], 1.3, 0.07, 0.025, "bpa_free_seal", "BPA & Phthalate Free Seal");
    ...
    registerDraggable(rhythmCard, [0, -96.43, 3.8], baseTilt, [0, 1.0, 1.0], 1.5, 0.08, 0.025, "daily_rhythm_card", "Daily Care Rhythm Card");
    ...
    registerDraggable(bathCard, [-6.5, -99.57, 1.8], baseTilt, [1.0, 1.0, 1.0], 1.3, 0.07, 0.02, "bath_time_card", "Bathing & Water Care Card");
    ...
    registerDraggable(crestBadge, [6.2, -100.54, 1.2], baseTilt, [-1.0, 1.0, 1.0], 1.4, 0.08, 0.025, "piwva_gold_crest", "Piwva Gold Royal Crest");
    ...
    registerDraggable(boardSeal, [-0.2, -103.58, 3.5], baseTilt, [0, 1.0, 1.0], 1.5, 0.08, 0.025, "pediatric_club_seal", "Pediatric Advisory Board Seal");
    ...
    const DEFAULT_CAM_POS = new THREE.Vector3(17, 12.47, 16.78);
    const DEFAULT_CAM_TARGET = new THREE.Vector3(0, 0, 0);

    // Exact user angle and zoom perspective when scrolling down
    const SCROLL_CAM_POS = new THREE.Vector3(17, 12.47, 16.78);
