...
const BENGALI_FONT =
  "'Hind Siliguri', 'Noto Sans Bengali', 'Plus Jakarta Sans', sans-serif";
...
export const Scene: React.FC<SceneProps> = ({
  ...
  useEffect(() => {
    const themeDef =
      TEXT_COLOR_THEMES[textColorTheme] || TEXT_COLOR_THEMES.gold;
    ...
    if (textSideMatRef.current) {
      ...
      textSideMatRef.current.metalness = Math.max(
        0.7,
        themeDef.metalness - 0.03,
      );
      textSideMatRef.current.roughness = Math.min(
        0.3,
        themeDef.roughness + 0.08,
      );
  ...
  useEffect(() => {
    ...
    if (!isIntroActiveRef.current) {
      camera.position.set(6.46, 10.6, 19.14);
    } else {
      camera.position.set(-4.42, 6.07, 9.11); // Exact User Intro Zoom Coordinates
    }
    ...
    const create3DEmblemBadge = (
      ...
      const logoPlane = new THREE.Mesh(
        new THREE.PlaneGeometry(width - 0.06, height - 0.06, 32, 16),
        logoMat,
      );
      ...
      loadPiwvaFont().then((font) => {
        ...
        text3D.mesh.position.set(
          0,
          -textSize * 0.05,
          depth / 2 + (textSize * 0.35) / 2 + 0.015,
        );
    ...
    const create3DPhotoCard = (
      ...
      const renderCard = () => {
        ...
        ctx.font = `bold 22px ${BENGALI_FONT}`;
        ctx.fillText(
          "✓ শিশু বিশেষজ্ঞদের সুপারিশকৃত • নিরাপদ মানদণ্ড",
          pad + 10,
          pad + imgH + 250,
        );
    ...
    const create3DHealthCard = (
      ...
      if (ctx) {
        ...
        ctx.textAlign = "center";
        ctx.fillText(
          "✓ শিশু বিশেষজ্ঞদের অনুমোদিত বৈজ্ঞানিক গাইডলাইন",
          512,
          botY + 34,
        );
      }
    ...
    const create3DCylinderMedallion = (
      icon: string,
      titleBengali: string,
      ...
      const tex = new THREE.CanvasTexture(canvas);
      const face = new THREE.Mesh(
        new THREE.CircleGeometry(rad - 0.04, 32),
    ...
    const create3DRingStacker = () => {
      ...
      const base = new THREE.Mesh(
        new THREE.CylinderGeometry(0.9, 1.0, 0.25, 32),
        new THREE.MeshPhysicalMaterial({ color: 0xdfb782, roughness: 0.3 }),
      );
      group.add(base);

      const peg = new THREE.Mesh(
        new THREE.CylinderGeometry(0.12, 0.12, 1.8, 24),
        new THREE.MeshPhysicalMaterial({ color: 0xc49b64, roughness: 0.3 }),
      );
      peg.position.y = 0.9;
      group.add(peg);
      ...
      const topSphere = new THREE.Mesh(
        ...
        new THREE.MeshPhysicalMaterial({ color: 0xdfb782, metalness: 0.85 }),
      );
    ...
    const create3DSiliconeChain = () => {
      const group = new THREE.Group();
      const beadCols = [
        0xdfb782, 0xccd5ae, 0xf4a261, 0xfaedcd, 0x78866b, 0xdfb782,
      ];
      beadCols.forEach((c, idx) => {
        const b = new THREE.Mesh(
          new THREE.SphereGeometry(0.2, 24, 24),
          new THREE.MeshPhysicalMaterial({ color: c, roughness: 0.3 }),
        );
    ...
    const renderScreenUI = () => {
      ...
      sContext.fillText(
        "🩺 শিশু বিশেষজ্ঞদের স্বাস্থ্য টিপস",
        bX + 220,
        bY + bH - 188,
      );
      ...
      sContext.textAlign = "left";
      sContext.fillText(
        "নবজাতকের স্বাস্থ্য ও যত্ন নির্দেশিকা",
        bX + 40,
        bY + bH - 120,
      );
      ...
      sContext.fillText(
        "নিরাপদ ঘুম, টামি টাইম ও কলিক উপশমের সঠিক নিয়ম",
        bX + 40,
        bY + bH - 68,
      );
      sContext.restore();
    ...
    const screenPlane = new THREE.Mesh(
      createRoundedPlaneGeometry(3.36, 7.16, 0.38, 16),
    ...
    const textRing = new THREE.Mesh(
      new THREE.TorusGeometry(2.5, 0.012, 16, 80),
      new THREE.MeshPhysicalMaterial({
        color: activeColorDef.ringColor,
        metalness: 0.95,
      }),
    );
    textRing.rotation.x = Math.PI / 3.2;
    piwvaTextGroup.add(textRing);
    const piwvaHitbox = new THREE.Mesh(
      new THREE.BoxGeometry(4.8, 1.8, 1.0),
    ...
    const logoJpgFace = new THREE.Mesh(
      new THREE.CircleGeometry(1.2, 48),
      new THREE.MeshBasicMaterial({ map: logoJpgTex }),
    );
    ...
    registerDraggable(
      ...
      0.08,
      0.03,
      "designed_logo_circle",
      "Piwva Designed Logo Medallion",
    );
    ...
    introAudio.addEventListener("canplaythrough", playIntroAudio, {
      once: true,
    });
    playIntroAudio();
    ...
    window.addEventListener("wheel", autoUnlockAudio, {
      once: true,
      passive: true,
    });
    window.addEventListener("scroll", autoUnlockAudio, {
      once: true,
      passive: true,
    });
    ...
    registerDraggable(
      ...
      0.025,
      "video_screen_circle",
      "Piwva Baby Video Reel",
    );

    // 5. Nursery Climate Card (Bengali)
    const palCard = create3DHealthCard(
      "নার্সারি ও রুমের সঠিক আবহাওয়া",
      "🌿 ১০০% নিরাপদ ও স্বাস্থ্যকর পরিবেশ",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "color_palette",
      "Nursery Climate Card",
    );

    // 6. Motor Milestones Card (Bengali)
    const motorCard = create3DHealthCard(
      "শারীরিক ও মস্তিষ্কের বিকাশ",
      "🩺 শিশু বিশেষজ্ঞ নির্দেশিকা • ০–৬ মাস",
      [
    ...
    registerDraggable(
      ...
      0.035,
      "product_card",
      "Motor & Sensory Health Card",
    );

    // 7. Health Guide Button Pill (Bengali)
    const snGroup = new THREE.Group();
    const snMesh = new THREE.Mesh(
      createRoundedBoxGeometry(3.0, 0.65, 0.1, 0.26, 8),
      new THREE.MeshPhysicalMaterial({ color: 0x94785c, roughness: 0.25 }),
    );
    snGroup.add(snMesh);
    ...
    const snFace = new THREE.Mesh(
      createRoundedPlaneGeometry(2.98, 0.63, 0.25, 16),
      new THREE.MeshBasicMaterial({ map: snTex }),
    );
    ...
    ["🩺", "❤️", "🌿"].forEach((ic, i) => {
      ...
      const icFace = new THREE.Mesh(
        new THREE.CircleGeometry(0.38, 32),
        new THREE.MeshBasicMaterial({ map: icTex }),
      );
    ...
    const block1 = new THREE.Mesh(
      createRoundedBoxGeometry(0.95, 0.95, 0.95, 0.12, 6),
      new THREE.MeshPhysicalMaterial({
        color: 0xdfb782,
        roughness: 0.28,
        metalness: 0.08,
    ...
    const block2 = new THREE.Mesh(
      createRoundedBoxGeometry(0.8, 0.8, 0.8, 0.1, 6),
      new THREE.MeshPhysicalMaterial({ color: 0xccd5ae, roughness: 0.3 }),
    );
    ...
    const rattleTorus = new THREE.Mesh(
      new THREE.TorusGeometry(0.85, 0.12, 24, 64),
      new THREE.MeshPhysicalMaterial({
        color: 0xdfb782,
        roughness: 0.25,
        clearcoat: 0.6,
    ...
    [0xf4a261, 0xccd5ae, 0xe76f51, 0xfaedcd].forEach((col, idx) => {
      ...
      const bMesh = new THREE.Mesh(
        ...
        new THREE.MeshPhysicalMaterial({ color: col, roughness: 0.35 }),
      );
    ...
    registerDraggable(
      ...
      0.03,
      "card_sleep_photo",
      "Sleeping Baby Photo Card",
    );

    // 14. Safe Sleep Tips (Bengali)
    const sleepTipsCard = create3DHealthCard(
      "নিরাপদ ঘুমের ৪টি মৌলিক নিয়ম",
      "😴 ০–১২ মাস বয়সের প্রোটোকল",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "moon_nightlight",
      "3D Moon & Stars Mobile",
    );

    // 16. Climate Medallion (Bengali)
    const tempBadge = create3DCylinderMedallion(
      "🌡️",
      "২০–২২°C",
      "৪৫–৫৫% আর্দ্রতা",
      "শ্বাসনালী সুরক্ষা",
    ...
    registerDraggable(
      ...
      0.02,
      "temp_badge",
      "Climate & Humidity Medallion",
    );

    // 17. Swaddle Safety Card (Bengali)
    const swaddleCard = create3DHealthCard(
      "সোয়াডলিং ও রূপান্তর গাইড",
      "🌿 নিরাপদ সোয়াডল নিয়ম",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "card_tummy_photo",
      "Tummy Time Baby Photo Card",
    );

    // 20. Tummy Tips (Bengali)
    const tummyTipsCard = create3DHealthCard(
      "টামি টাইমের মূল লক্ষ্য ও নিয়ম",
      "🍼 ০–৬ মাস শারীরিক বিকাশ",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "card_crawling_photo",
      "Crawling Baby Photo Card",
    );

    // 22. Pediatrician Endorsement (Bengali)
    const reviewCard = create3DHealthCard(
      "শিশু বিশেষজ্ঞের ক্লিনিক্যাল সুপারিশ",
      "⭐ ৫.০ মেডিকেল পরামর্শ",
      [
    ...
    registerDraggable(
      ...
      0.02,
      "parent_review",
      "Pediatric Endorsement Card",
    );

    // 23. High-Contrast Vision (Bengali)
    const contrastCard = create3DHealthCard(
      "হাই-কনট্রাস্ট দৃষ্টিশক্তি গাইড",
      "👀 স্নায়বিক উদ্দীপনা",
      [
    ...
    const abFrame = new THREE.Mesh(
      createRoundedBoxGeometry(2.2, 1.8, 0.15, 0.1, 6),
      new THREE.MeshPhysicalMaterial({ color: 0xdfb782, roughness: 0.3 }),
    );
    ...
    [-0.4, 0, 0.4].forEach((y, r) => {
      ...
      [-0.5, -0.2, 0.1, 0.4].forEach((x, b) => {
        const bead = new THREE.Mesh(
          ...
          new THREE.MeshPhysicalMaterial({
            color: [0xef4444, 0x3b82f6, 0x10b981, 0xf59e0b][(r + b) % 4],
          }),
        );
    ...
    registerDraggable(
      ...
      0.03,
      "wooden_abacus_3d",
      "3D Montessori Abacus Model",
    );

    // 25 & 26. Milestones Badges (Bengali)
    const m3Badge = create3DCylinderMedallion(
      "✨",
      "৩ মাসের লক্ষ্য",
      "ঘাড় শক্ত ও হাসিমুখ",
      "মেডিকেল চেক",
    ...
    registerDraggable(
      ...
      0.02,
      "milestone_3m_card",
      "3-Month Milestones Badge",
    );

    const m6Badge = create3DCylinderMedallion(
      "🌟",
      "৬ মাসের লক্ষ্য",
      "সাপোর্ট নিয়ে বসা",
      "মোটর মাইলস্টোন",
    ...
    registerDraggable(
      ...
      0.03,
      "card_teething_photo",
      "Teething Baby Photo Card",
    );

    // 28. Teething Tips (Bengali)
    const teethingTipsCard = create3DHealthCard(
      "টিথিং উপশমের নিরাপদ উপায়",
      "🌱 নিরাপদ টিথিং প্রোটোকল",
      [
    ...
    registerDraggable(
      ...
      0.025,
      "silicone_beads_3d",
      "3D Silicone Teether Chain",
    );

    // 30. Eco Badge (Bengali)
    const ecoBadge = create3DCylinderMedallion(
      "🌱",
      "১০০% নিরাপদ",
      "এফএসসি বিচউড",
      "টক্সিক উপাদান-মুক্ত",
    ...
    registerDraggable(
      ...
      0.025,
      "eco_badge",
      "100% Newborn Safe Health Seal",
    );

    // 31. Solids Guide (Bengali)
    const solidsCard = create3DHealthCard(
      "প্রথম শক্ত খাবার ও উইনিং গাইড",
      "🥑 ৬+ মাস পুষ্টি শুরু",
      [
    ...
    registerDraggable(
      ...
      0.02,
      "solids_guide_card",
      "Weaning & Solids Guide Card",
    );

    // 32. Wooden Spoon
    const spoonGroup = new THREE.Group();
    const handle = new THREE.Mesh(
      new THREE.CylinderGeometry(0.04, 0.06, 1.6, 16),
      new THREE.MeshPhysicalMaterial({ color: 0xdfb782, roughness: 0.25 }),
    );
    ...
    registerDraggable(
      ...
      0.03,
      "wooden_spoon_3d",
      "3D Wooden Weaning Spoon",
    );

    // 33. Oral Hygiene (Bengali)
    const oralWipeCard = create3DHealthCard(
      "মাড়ি ও দাঁতের যত্ন প্রোটোকল",
      "🪥 দৈনিক মুখের পরিচ্ছন্নতা",
      [
    ...
    const starToy = new THREE.Mesh(
      new THREE.CylinderGeometry(0.65, 0.65, 0.15, 5),
      new THREE.MeshPhysicalMaterial({
        color: 0xf4a261,
        roughness: 0.35,
        clearcoat: 0.3,
    ...
    registerDraggable(
      ...
      0.03,
      "card_mother_photo",
      "Kangaroo Care Baby Photo Card",
    );

    // 36. Colic Relief (Bengali)
    const colicTipsCard = create3DHealthCard(
      "কলিক ও পেটের গ্যাস নিরাময়",
      "🍼 পরিপাকতন্ত্রের আরাম",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "cloud_mobile_3d",
      "3D Cloud Nursery Mobile",
    );

    // 38. Newsletter Card (Bengali)
    const newsCard = create3DHealthCard(
      "সাপ্তাহিক স্বাস্থ্য টিপস ক্লাব",
      "💌 নিয়মিত স্বাস্থ্য পরামর্শ",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "newsletter_card",
      "Pediatrician Health Tips Card",
    );

    // 39. Burping Techniques (Bengali)
    const burpingCard = create3DHealthCard(
      "ঢেকুর তোলানোর ৩টি সহজ পদ্ধতি",
      "🍼 ফিডিং কমফোর্ট",
      [
    ...
    registerDraggable(
      ...
      0.02,
      "burping_guide_card",
      "Burping Techniques Guide Card",
    );

    // 40. Massage Routine (Bengali)
    const massageCard = create3DHealthCard(
      "শিশুর বডি ম্যাসাজ রুটিন",
      "🌿 প্রশান্তি ও আরামদায়ক ঘুম",
      [
    ...
    registerDraggable(
      ...
      0.025,
      "massage_oil_card",
      "Infant Massage Routine Card",
    );

    // 41. Kangaroo Badge (Bengali)
    const kangarooBadge = create3DCylinderMedallion(
      "❤️",
      "স্কিন-টু-স্কিন",
      "হৃদস্পন্দন নিয়ন্ত্রণ",
      "আবেগময় বন্ধন",
    );
    registerDraggable(
      kangarooBadge,
    ...
    const feverCard = create3DHealthCard(
      "নবজাতকের জ্বর সতর্কতা গাইড",
      "🚨 জরুরি মেডিকেল প্রোটোকল",
      [
    ...
    registerDraggable(
      ...
      0.03,
      "fever_protocol_card",
      "Infant Fever Protocol Card",
    );

    // 44. Dehydration Checklist (Bengali)
    const dehydrationCard = create3DHealthCard(
      "ডিহাইড্রেশনের ক্লিনিক্যাল লক্ষণ",
      "💧 পানিশূন্যতা চেকলিস্ট",
      [
    ...
    registerDraggable(
      ...
      0.025,
      "dehydration_card",
      "Dehydration Warning Signs Card",
    );

    // 45. Respiratory Signs (Bengali)
    const breathingCard = create3DHealthCard(
      "শ্বাসকষ্টের সতর্কতা সংকেত",
      "🫁 শ্বাসপ্রশ্বাস পর্যবেক্ষণ",
      [
    ...
    const thermoBody = new THREE.Mesh(
      new THREE.CapsuleGeometry(0.12, 1.2, 16, 16),
    ...
    const thermoTip = new THREE.Mesh(
      ...
      new THREE.MeshPhysicalMaterial({ color: 0xdfb782, metalness: 0.95 }),
    );
    ...
    registerDraggable(
      ...
      0.03,
      "thermometer_3d",
      "3D Pediatric Thermometer",
    );

    // 47. Stethoscope Badge (Bengali)
    const stethBadge = create3DCylinderMedallion(
      "🩺",
      "ক্লিনিক্যাল পরামর্শ",
      "শিশু বিশেষজ্ঞ অনুমোদিত",
      "সঠিক চিকিৎসা মানদণ্ড",
    );
    registerDraggable(
      stethBadge,
      [6.8, -84.2, 1.2],
    ...
    const kitBox = new THREE.Mesh(
      createRoundedBoxGeometry(1.6, 1.2, 0.8, 0.15, 6),
      new THREE.MeshPhysicalMaterial({
        color: 0xffffff,
        roughness: 0.15,
        clearcoat: 0.4,
    ...
    const cross1 = new THREE.Mesh(
      new THREE.BoxGeometry(0.6, 0.18, 0.02),
      new THREE.MeshBasicMaterial({ color: 0xef4444 }),
    );
    cross1.position.z = 0.41;
    const cross2 = new THREE.Mesh(
      new THREE.BoxGeometry(0.18, 0.6, 0.02),
      new THREE.MeshBasicMaterial({ color: 0xef4444 }),
    );
    ...
    const fscSeal = create3DCylinderMedallion(
      "🌲",
      "১০০% প্রাকৃতিক",
      "এফএসসি বিচউড",
      "লেড ও কেমিক্যাল-মুক্ত",
    );
    registerDraggable(
      fscSeal,
      [-5.5, -92.0, 3.0],
      ...
      0.025,
      "fsc_wood_seal",
      "FSC Beechwood Certified Seal",
    );

    // 50. BPA-Free Seal (Bengali)
    const bpaSeal = create3DCylinderMedallion(
      "🛡️",
      "বিপিএ-মুক্ত",
      "ল্যাব টেস্টে নিরাপদ",
      "শিশুর শতভাগ সুরক্ষা",
    ...
    registerDraggable(
      ...
      0.025,
      "bpa_free_seal",
      "BPA & Phthalate Free Seal",
    );

    // 51. Daily Rhythm Card (Bengali)
    const rhythmCard = create3DHealthCard(
      "নবজাতকের দৈনিক যত্নের রুটিন",
      "⏰ নিয়মমাফিক জীবনধারা",
      [
    ...
    registerDraggable(
      ...
      0.025,
      "daily_rhythm_card",
      "Daily Care Rhythm Card",
    );

    // 52. Bathing Card (Bengali)
    const bathCard = create3DHealthCard(
      "কুসুম গরম পানিতে গোসলের নিয়ম",
      "🛁 নিরাপদ গোসল গাইড",
      [
    ...
    registerDraggable(
      ...
      0.02,
      "bath_time_card",
      "Bathing & Water Care Card",
    );

    // 53. Piwva Gold Crest (Bengali)
    const crestBadge = create3DCylinderMedallion(
      "👑",
      "PIWVA এক্সিলেন্স",
      "অর্গানিক ও নিরাপদ যত্ন",
      "মন্টেশ্বরী স্ট্যান্ডার্ড",
      true,
      0.95,
    );
    registerDraggable(
      ...
      0.025,
      "piwva_gold_crest",
      "Piwva Gold Royal Crest",
    );

    // 54. Pediatric Board Seal (Bengali)
    const boardSeal = create3DCylinderMedallion(
      "🩺",
      "মেডিকেল বোর্ড",
      "শিশু বিশেষজ্ঞ সিল",
      "ক্লিনিক্যাল সুরক্ষা",
    ...
    const findTargetData = (
      obj: THREE.Object3D | null,
    ): DraggableObjectData | null => {
    ...
    const updateCtrlState = (held: boolean) => {
      if (isCtrlHeldRef.current !== held) {
        ...
        controls.enableRotate =
          !held && !isSpaceHeldRef.current && !isIntroActiveRef.current;
        controls.enablePan =
          !held && !isSpaceHeldRef.current && !isIntroActiveRef.current;
    ...
    const onKeyUp = (e: KeyboardEvent) => {
      if (e.code === "Space" || e.key === " ") {
        if (isSpaceHeldRef.current) {
          ...
          onSpaceChange?.(false);
          controls.enableRotate =
            !isCtrlHeldRef.current && !isIntroActiveRef.current;
          controls.enablePan =
            !isCtrlHeldRef.current && !isIntroActiveRef.current;
          renderer.domElement.style.cursor = isCtrlHeldRef.current
            ? "grab"
            : "default";
        }
      }
    ...
    const onPointerDown = (e: MouseEvent | TouchEvent) => {
      ...
      const isCtrl =
        "ctrlKey" in e
          ? e.ctrlKey || e.metaKey || isCtrlHeldRef.current
          : isCtrlHeldRef.current;
      if (!isCtrl) return;
    ...
    const onPointerMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (isPanningArtboardRef.current && isSpaceHeldRef.current) {
        ...
        const factor =
          (dist * Math.tan((camera.fov * Math.PI) / 360) * 2) /
          window.innerHeight;

        const camDir = new THREE.Vector3();
        camera.getWorldDirection(camDir);
        const panRight = new THREE.Vector3()
          .crossVectors(camera.up, camDir.clone().negate())
          .normalize();
        const panUp = camera.up.clone().normalize();

        const panDelta = panRight
          .clone()
          .multiplyScalar(-dx * factor)
          .add(panUp.clone().multiplyScalar(dy * factor));
      ...
      if (activeDragged.current && isDraggingAny.current) {
        ...
        if (intersects.length > 0) {
          ...
          if (targetData) {
            ...
            renderer.domElement.style.cursor = "grab";
            draggableObjects.current.forEach(
              (d) => (d.isHovered = d === targetData),
            );
          } else {
            renderer.domElement.style.cursor = "default";
            draggableObjects.current.forEach((d) => (d.isHovered = false));
    ...
    const onPointerUp = (e: MouseEvent | TouchEvent) => {
      if (isPanningArtboardRef.current) {
        isPanningArtboardRef.current = false;
        renderer.domElement.style.cursor = isSpaceHeldRef.current
          ? "grab"
          : "default";
        soundFX.playPop(480);
      }
      ...
      if (activeDragged.current) {
        ...
        const isCtrl =
          "ctrlKey" in e
            ? e.ctrlKey || e.metaKey || isCtrlHeldRef.current
            : isCtrlHeldRef.current;
        renderer.domElement.style.cursor = isCtrl
          ? "grab"
    ...
    const animate = () => {
      ...
      if (isIntroActiveRef.current) {
        ...
        if (t < SLOW_PHASE_DURATION) {
          ...
          const ease =
            p < 0.5 ? 4 * p * p * p : 1 - Math.pow(-2 * p + 2, 3) / 2; // smooth cubic ease in-out
          ...
          controls.enableRotate =
            !isCtrlHeldRef.current && !isSpaceHeldRef.current;
          controls.enablePan =
            !isCtrlHeldRef.current && !isSpaceHeldRef.current;
        }
        controls.update();
      ...
      const targetProgress =
        scrollProgressPropRef.current !== undefined
      ...
      const scrollZoom = 1.0 + currentScrollProgress * 0.3;
      ...
      draggableObjects.current.forEach((obj) => {
        ...
        const floatY = isThisDragged
          ? 0
          : Math.sin(time * obj.floatSpeed + obj.phase) * obj.floatAmp;
        const floatRotZ = isThisDragged
          ? 0
          : Math.cos(time * obj.floatSpeed * 0.8 + obj.phase) * obj.rotFloatAmp;
        const floatRotX = isThisDragged
          ? 0
          : Math.sin(time * obj.floatSpeed * 0.7 + obj.phase) *
            (obj.rotFloatAmp * 0.6);
        ...
        obj.mesh.rotation.x = THREE.MathUtils.lerp(
          obj.mesh.rotation.x,
          obj.initialRot.x + floatRotX,
          delta * 8,
        );
        obj.mesh.rotation.y = THREE.MathUtils.lerp(
          obj.mesh.rotation.y,
          obj.initialRot.y,
          delta * 8,
        );
        obj.mesh.rotation.z = THREE.MathUtils.lerp(
          obj.mesh.rotation.z,
          obj.initialRot.z + floatRotZ,
          delta * 8,
        );
      ...
      separateLettersList.forEach((letter) => {
        letter.mesh.position.y =
          letter.baseY + Math.sin(time * 2.4 + letter.phase) * 0.09;
