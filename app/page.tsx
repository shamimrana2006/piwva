...
export default function Home() {
  ...
  const [lightingTheme, setLightingTheme] = useState<
    "studio" | "warm" | "cool"
  >("studio");
  const [bgTheme, setBgTheme] = useState<
    "dark-starry" | "light-soft" | "warm-nebula"
  >("dark-starry");
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (track3DRef.current) {
        const trackHeight =
          track3DRef.current.offsetHeight - window.innerHeight;
        if (trackHeight > 0) {
  ...
  const getStageInfo = () => {
    if (scrollProgress < 0.12) {
      return {
        stage: "পর্ব ০",
        title: "🌿 নার্সারি ও ব্র্যান্ড ভিডিও রিল",
        color: "bg-emerald-100/90 text-emerald-800 border-emerald-300",
      };
    } else if (scrollProgress < 0.26) {
      return {
        stage: "পর্ব ১",
        title: "🌙 নিরাপদ ঘুম ও সিডস প্রতিরোধ",
        color: "bg-blue-100/90 text-blue-800 border-blue-300",
      };
      ...
      return {
        stage: "পর্ব ৩",
        title: "🦷 টিথিং, মাড়ির যত্ন ও প্রথম উইনিং",
        color: "bg-purple-100/90 text-purple-800 border-purple-300",
      };
    } else if (scrollProgress < 0.74) {
      return {
        stage: "পর্ব ৪",
        title: "❤️ ক্যাঙ্গারু কেয়ার ও কলিক উপশম",
        color: "bg-rose-100/90 text-rose-800 border-rose-300",
      };
    } else if (scrollProgress < 0.88) {
      return {
        stage: "পর্ব ৫",
        title: "🚨 জরুরি স্বাস্থ্য সতর্কতা লক্ষণ",
        color: "bg-red-100/90 text-red-800 border-red-300",
      };
    } else {
      return {
        stage: "পর্ব ৬",
        title: "🏆 এফএসসি সার্টিফাইড ও দৈনিক রুটিন",
        color: "bg-teal-100/90 text-teal-800 border-teal-300",
      };
    }
  };
  ...
  return (
    <main
      className={`relative min-h-screen w-full ${bgTheme === "dark-starry" ? "bg-[#0c1517]" : bgTheme === "warm-nebula" ? "bg-[#181412]" : "bg-[#c5d5d3]"} overflow-x-hidden selection:bg-[#dfb782]/30 transition-colors duration-500`}
    >
      ...
      <div className="fixed top-4 left-6 z-40 hidden sm:flex items-center gap-2.5">
        <div
          className={`px-4 py-2 rounded-2xl border backdrop-blur-md shadow-md text-xs font-bold transition-all duration-300 flex items-center gap-2 ${currentStage.color}`}
        >
      ...
      >
        ...
        <div className="pt-24 flex justify-center">
          <div className="bg-white/85 backdrop-blur-md px-5 py-2 rounded-full border border-white shadow-sm text-xs font-bold text-[#1e293b] uppercase tracking-wider flex items-center gap-2 animate-bounce">
            <span>
              নিচে স্ক্রল করে ৫৪+ ৩ডি স্বাস্থ্য কার্ড, ভিডিও রিল ও শিশুর ছবি
              আবিষ্কার করুন ↓
            </span>
          </div>
        </div>
        ...
        <div className="pb-16 flex justify-center">
          <div className="bg-white/90 backdrop-blur-md px-5 py-2 rounded-full border border-white shadow-md text-xs font-bold text-[#15803d] tracking-wider flex items-center gap-2">
            <span>
              🏆 ৫৪টি ৩ডি উপাদান সমাপ্ত • নিচে বিস্তারিত স্বাস্থ্য গাইড দেখুন ↓
            </span>
          </div>
        </div>
      </div>
